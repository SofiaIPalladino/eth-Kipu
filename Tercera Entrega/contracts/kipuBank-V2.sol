...
import "https://raw.githubusercontent.com/OpenZeppelin/openzeppelin-contracts/v4.9.3/contracts/utils/math/Math.sol";
...
contract KipuBankV2 is AccessControl, ReentrancyGuard, Pausable {
    ...
    /// @notice Scale factor used to convert values to 18 decimals (USD18)
    ...
    /// @notice Emitted when tokens are moved internally between accounts
    ...
    /// @notice Revert cuando la multiplicación price * USD18_SCALE excede uint256
    error PriceMultiplierOverflow();
    ...
    constructor(
        ...
        // register ETH feed (decimals típicamente 8)
        ...
        onlySupportedToken(token)
    {
        ...
        // Emit a clear internal transfer event
        emit InternalTransfer(msg.sender, to, token, amountRaw);
    }
    ...
    */
    function _convertToUsd18(address token, uint256 amountRaw) internal view returns (uint256 usd18) {
        ...
        // denom = 10^(tokenDecimals + priceDecimals)
        uint256 denom = 10 ** (uint256(td) + uint256(priceDecimals));

        // Prevent overflow when computing price * USD18_SCALE
        if (price > type(uint256).max / USD18_SCALE) revert PriceMultiplierOverflow();

        // Compute numerator multiplier safely once price * 1e18
        uint256 numeratorMultiplier = price * USD18_SCALE;

        // Use Math.mulDiv to perform amountRaw * numeratorMultiplier / denom with full precision and overflow-safety
        usd18 = Math.mulDiv(amountRaw, numeratorMultiplier, denom);
    }
    ...
    * @dev Revertará si el nuevo total diario excede `perUserDailyWithdrawLimitUsd18`
    */
